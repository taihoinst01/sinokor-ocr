...
for key in legacyArr['LEGACY']:
    # if (legacyItem[key] is None):
    #     print('')
    if (legacyArr['LEGACY'][key] != None and legacyArr['LEGACY'][key].strip() != '0.00'):
        maxaccu = 0
        maxText = ''
        maxLocation = ''
        for inputItem in inputArr:
            accu = get_cosine(text_to_vector(legacyArr['LEGACY'][key].lower()), text_to_vector(inputItem['text'].lower()))
            if (accu > maxaccu):
        ...
        if (maxaccu > 0.5):
            ...
            tempDict['sid'] = maxSid
            tempDict['filepath'] = legacyArr['FILEPATH']
            tempDict['imgid'] = legacyArr['IMGID']
