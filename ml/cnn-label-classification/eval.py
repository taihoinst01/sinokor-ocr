...
if FLAGS.eval_train:
    ...
    x_raw, y_test = data_loader.load_dev_data_and_labels_test(textList)
    y_test = np.argmax(y_test, axis=1)

# checkpoint_dir이 없다면 가장 최근 dir 추출하여 셋팅
if FLAGS.checkpoint_dir == "":
    all_subdirs = ["./ml/cnn-label-classification/runs/" + d for d in os.listdir('./ml/cnn-label-classification/runs/.') if os.path.isdir("./ml/cnn-label-classification/runs/" + d)]
...
#     csv.writer(f).writerows(predictions_human_readable)vd