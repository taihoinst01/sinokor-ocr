...
def findLabelDB(rows, inputsid):
    ...
    for row in rows:
        if int(row[2]) != 38 and (int(row[2]) < 3 or int(row[2]) > 34):
            ...
            if (boundaryCheck(dbNum[1], inputNum[1]) and (
                    boundaryCheck(dbNum[0], inputNum[0]) or boundaryCheck(dbNum[2], inputNum[2]))):
                ret.append(row[2])
...
def classifyForm(inputArr):
    formNum = 0

    formSid = []
    for inputItem in inputArr:
        sid = inputItem['sid'].split(",")[-5:]
    ...
    sqlForm = ("SELECT * FROM TBL_FORM_MAPPING WHERE DATA = :data ")
    curs.execute(sqlForm, data = sidData)
    formCls = curs.fetchall()

    if len(formCls) == 0:
        formNum = 0
    else:
        formNum = formCls['class']
...
def eval(inputJson):
    ...
    try:
        ######################################
        # 문서분류용 테이블 TBL_FORM_MAPPING 에 inputArr에 있는 상위 5개의 문장의 sid 추출 후 Data 필드 조회
        # 결과가 없으면 return 0
        # 계산서가 아니면 return 1
        # 저장된 계산서는 2부터

        formNum = classifyForm(inputArr)

        ##########################################
        # 문서분류가 1이면 아래 로직 패스
        if formNum < 2:
            print(str({'code': 500, 'message': 'not mapping form'}))
            return
