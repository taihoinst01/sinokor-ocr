...
exports.selectDocument = function (req, done) {
    return new Promise(async function (resolve, reject) {
        ...
        try {
            conn = await oracledb.getConnection(dbConfig);
            result = await conn.execute(`SELECT SEQNUM, DOCNUM, PAGECNT, STATUS, DRAFTERNUM FROM TBL_APPROVAL_MASTER WHERE DOCNUM IN ( ` + req + `)`);
            if (result.rows.length > 0) {
                return done(null, result.rows);
            } else {
                return done(null, []);
...
exports.insertDocument = function (req, done) {
    return new Promise(async function (resolve, reject) {
        ...
        try {
            conn = await oracledb.getConnection(dbConfig);
            await conn.execute(`INSERT INTO
                                    TBL_APPROVAL_MASTER(SEQNUM, DOCNUM, PAGECNT, STATUS, MEMO, DRAFTERNUM)
                                VALUES 
                                    (SEQ_DOCUMENT.NEXTVAL, :docNum, :pageCnt, 'ZZ', :memo, :drafterNum) `, [req[0][0].imgId, req[0].length, req[0][0].oriFileName, req[0][0].regId]);            
