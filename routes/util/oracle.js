...
exports.insertColumnMapping = function (req, done) {
    return new Promise(async function (resolve, reject) {
        ...
        try {
            ...
            //20180903 hskim ë§Œì•½ colLbl??3 ?ì„œ 34?¬ì´(?ìŠ¤?¸ë¡œ êµ¬ë¶„?˜ëŠ”ì¹¼ëŸ¼) ?´ë©´??sidê°€ 0,0,0,0,0 ?´ê±°??1,1,1,1,1 ?´ë©´ ?¸ì„œ??ê¸ˆì?
...
exports.insertOcrSymsSingle = function (req, done) {
    return new Promise(async function (resolve, reject) {
        ...
        try {
            ...
            var regExp = /[\{\}\[\]\/?.,;:|\)*~`!^\-_+<>@\#$%&\\\=\(\'\"]/gi;
            for (var i in reqArr) {
                //20180903 ë§Œì•½ ?¨ì–´ê°€ ê¸°í˜¸ë¡œë§Œ ?´ë£¨?´ì ¸ ?ˆìœ¼ë©??¸ì„œ???ˆí•¨
                result = await conn.execute(selectTypo, [reqArr[i].replace(regExp, "")]);
                if (result.rows.length == 0) {
                    //?«ìžë§??ˆì„?ŒëŠ” insert ?ˆí•¨
    ...
    return new Promise(async function (resolve, reject) {
        ...
        try {
            ...
            if (selContract.rows.length == 0 && selContractAsog.rows.length == 0) {
                ...
                //201880903 check ì¡°íšŒ ???ˆì„ê²½ìš° ?¸ì„œ???ˆí•¨
