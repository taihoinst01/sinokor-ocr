...
exports.columnMapping3 = function (data, callback) {
    sync.fiber(function () {
        ...
        //var resPyStr = sync.await(PythonShell.run('columnClassicifyFromAzure.py', pythonConfig.columnMappingOptions, sync.defer())); //azure
        var resPyArr = JSON.parse(resPyStr[0].replace(/'/g, '"'));
        if (!resPyArr.code || (resPyArr.code && resPyArr.code == 200)) {
            data = resPyArr;
            callback(data);
...
exports.addTrainFromMLStudio = function (data, callback) {
    sync.fiber(function () {
        ...
        arg = {};
        arg.data = data;
        var result = sync.await(mlStudio.train(arg, sync.defer()));
